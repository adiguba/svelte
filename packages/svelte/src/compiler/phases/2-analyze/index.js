...
export function analyze_component(root, source, options) {
	...
	for (const node of analysis.elements) {
		...
		for (const attribute of node.attributes) {
			...
			has_style_directive ||= attribute.type === 'StyleDirective';
			if (attribute.type == 'Attribute') {
				const name = attribute.name.toLowerCase();
				has_class ||= name === 'class';
				has_style ||= name === 'style';
			}
		}

		if (!has_spread) {
			// We need an empty class to generate the set_class() or class="" correctly
			if (!has_class && (node.metadata.scoped || has_class_directive)) {
			...
			// We need an empty class to generate the set_style() correctly
			if (!has_style && has_style_directive) {
				node.attributes.push(
