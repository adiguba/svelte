...
export function SvelteElement(node, context) {
	...
	let is_only_class_or_style = true;
	for (const attribute of attributes) {
		if (attribute.type === 'Attribute') {
			const name = attribute.name.toLowerCase();
			if (name !== 'class' && name !== 'style') {
				is_only_class_or_style = false;
				break;
			}
		}
	}

	if (is_only_class_or_style && attributes.length) {
		// special case when there only class/style attribute
		for (const attribute of /** @type {AST.Attribute[]} */ (attributes)) {
			...
			if (attribute.name === 'class') {
				...
				build_set_style(element_id, value, has_state, style_directives, inner_context);
			} else {
